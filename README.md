...
Measured against a live upstream `watermarks-remover`, not assumed.

| Upload | Cleaned by | Result |
| --- | --- | --- |
| `.txt` | **the Worker** (Layer A) | ✅ invisible Unicode removed, **no container, no model, no network** |
| `.md`, `.html` | container (`container` kind) | ✅ Layer A applied, AI frontmatter/meta stripped |
| `.png`, `.jpg`, … | container (`image` kind) | ✅ metadata stripped via exiftool; C2PA/AI flags reported |
| `.pdf` | container (`container` kind) | ⚠️ works, but degraded without `exiftool`/`qpdf` on the cleaner |

**Why `.txt` is handled in the Worker.** Upstream routes plain text to its
`text` kind and makes a **Layer B LLM rewrite mandatory** there, rejecting
`/clean` with `Layer B strategy needs an LLM rewrite backend` when no model is
configured. Layer A needs no model, so the Worker applies it directly — the
plan `containers/cleaner/README.md` set out for v1 ("the text stays local;
Layer B = v1.5"). `apps/worker/src/layer-a/` is a **faithful port** of
upstream's `clean_text()`; `tests/layer-a.test.ts` diffs it against 2560 cases
generated by the real Python implementation, so the two agree
character-for-character.

That fidelity matters: ZWJ, variation selectors and tag characters are
**load-bearing** inside emoji (❤️‍🔥, 👨‍👩‍👧, 🏴󠁧󠁢󠁳󠁣󠁴󠁿), Persian and Devanagari
orthography, and bidi runs. A naive "strip everything invisible" corrupts real
text; this does not.

**El `.txt` lo limpia el propio Worker** con un port fiel de la Capa A de
upstream, sin contenedor y sin modelo. Upstream exige Capa B (LLM) para texto
plano; la Capa A no la necesita. El port se verifica contra 2560 casos generados
por la implementación Python real.
